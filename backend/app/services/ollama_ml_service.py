...
def _request_chat(prompt: str, *, json_mode: bool = False) -> str:
    ...
    with urllib.request.urlopen(request, timeout=settings.ollama_timeout_sec) as response:
...
def analyze_review(text: str, limit: int = 3) -> tuple[SentimentResult, list[str]]:
    """Single Ollama call: sentiment + topics for one review."""
    prompt = (
...
def generate_summary(
    ...
    """Llama-generated course summary from review corpus."""
    if not review_texts:
